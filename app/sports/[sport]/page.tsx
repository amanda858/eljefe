...
export default async function SportHubPage({ params }: SportHubPageProps) {
    ...
    const initialIntel: Array<{ id: string; title: string; tag: string; time: string; detail: string }> = [];
    const sportEvents = realEvents;
    const seasonFeed = await getSeasonFeedForSport(hub.sport);
    const slateRows = todayGames.filter((game) => game.leagueCode === hub.sport).map(mapGameToSlateRow);
    const featuredEdges = todayEdges.filter((edge) => edge.sport === hub.sport).slice(0, 3).map(mapEdgeToPreviewRow);
    ...
    return (
        <main className="page-shell sports-shell">
            ...
            <section className="content-section sports-headlines">
                <div className="section-heading">
                    <p className="eyebrow">Best current edges</p>
                    <h2>Backend-reviewed positions for {hub.sport} right now.</h2>
                </div>
                <div className="sports-hub-grid">
                    {featuredEdges.length > 0 ? featuredEdges.map((edge) => (
                        <article className="system-card" key={edge.id}>
                            <p className="card-kicker">{edge.edge} · {edge.edgeScore} score</p>
                            <h3>{edge.game}</h3>
                            <p>{edge.bestLine}</p>
                        ...
                        <article className="season-empty-state">
                            <p className="card-kicker">No live backend edges</p>
                            <h3>This sport currently has no positive backend-reviewed edge rows.</h3>
                            <p className="market-detail-copy">When the market engine surfaces value here, this section will populate automatically.</p>
                        </article>
                    )}
                </div>
            </section>
            ...
            <section className="content-section">
                <div className="section-heading">
                    <p className="eyebrow">Sport slate</p>
                    <h2>Records, weather, and direct drill-in for {hub.sport} matchups.</h2>
                </div>
            ...
            <section className="content-section closing-section">
                <div>
                    ...
                    <h2>Move across sports or view all edges.</h2>
                </div>
                <div className="closing-actions">
                    <Link className="button button-primary" href="/edges">
                        View best edges
                    </Link>
